...
class Distributor():
    def __init__(self, sync_addr:tuple, auth:str) -> None:
        ...
        self.pool_hosts = {
            # This new entry points to our local mock stratum pool server.
            'fg_stratum_pool': ('127.0.0.1', 3333),
        ...
        # Default to our new mock pool
        self.current_pool = 'fg_stratum_pool'
    ...
    def run_bg_tasks(self):
        if self.connect_to_sync_server():
            ...
            if not self.stratum_client.connected:
                self.activity.appendleft({"time":datetime.now().strftime('%H:%M:%S'),"node":f"Failed to connect to {self.p2pool_host}:{self.p2pool_port}","is_event":True})
                self.server_running = False
                return

            self.activity.appendleft({"time":datetime.now().strftime('%H:%M:%S'),"node":f"Connected to Stratum pool at {self.p2pool_host}:{self.p2pool_port}","is_event":True})
                ...
                if msg.get('lodging'): self.register_client(msg)
                    ...
                    if self.latest_pool_job:
                        try:
                            ...
                            self.activity.appendleft({"time":datetime.now().strftime('%H:%M:%S'), "node": f"Job {client_job.get('job_id','')[:8]}... sent to client {msg['client_id'][:8]}..."})
                        except Exception as e:
